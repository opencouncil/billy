...
class AgendasTestCase(BaseTestCase):

    def setUp(self):
        '''Load test data. The API only returns future agendas, so we need
        to set the 'when' attribute of the test agenda to a future date.
        '''
        BaseTestCase.setUp(self)
        future_data = datetime.datetime.now() + datetime.timedelta(weeks=1)
        spec = getattr(self, 'data', {})
        self.db.agendas.find_and_modify(spec, {'$set': {'when': future_data}})


class AgendasSearchTestCase(AgendasTestCase):

    url_tmpl = '/api/v1/agendas/'
    data = dict(state='ex')

    def test_count(self):
        self.assertEquals(
            len(self.json),
            self.db.agendas.find(self.data).count())
...
class AgendaLookupTestCase(AgendasTestCase):

    url_tmpl = '/api/v1/agendas/{agenda_id}/'
    url_args = dict(agenda_id='EXE00000001')
